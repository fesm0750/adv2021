...
pub fn run() {
    ...
    let (len_x, len_y) = input
        .iter()
        .fold((0, 0), |(lx, ly), b| (lx.max(b.x), ly.max(b.y)));
    ...
    let count_overlaps_diagonal = overlaps_diagonal_lines(&mut grid, &input);

    println!("Day 05");
    println!(
        "Count of overlaps for straight lines: {}",
        count_overlaps_straight
    );
    println!(
        "Count of overlaps for all lines: {}",
        count_overlaps_diagonal
    );
    println!();
}
...
/// returns an iterator yielding a point at a time
...
/// helper method to update a row line in the grid based on the coordinates given by p0 and p1. The
/// y coordinate from p0 is soleny used the determine the column.
...
/// helper method to update any kind of line in the grid (rows, columns or diagonals at 45°), based
/// on the coordinates given by points p0 and p1.
fn fill_line(grid: &mut Grid<u16>, p0: &Base2d<u16>, p1: &Base2d<u16>) {
